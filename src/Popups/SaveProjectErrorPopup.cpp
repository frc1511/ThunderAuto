...
void SaveProjectErrorPopup::present(bool* running) {
  ImGui::SetNextWindowPos(ImGui::GetMainViewport()->GetCenter(), false, ImVec2(0.5f, 0.5f));
  ImGui::SetNextWindowSize(ImVec2(GET_UISIZE(PROJECT_SAVE_ERROR_POPUP_START_WIDTH),
                                  GET_UISIZE(PROJECT_SAVE_ERROR_POPUP_START_HEIGHT)),
                           ImGuiCond_FirstUseEver);

  ImGui::SetNextWindowSizeConstraints(ImVec2(0, GET_UISIZE(PROJECT_SAVE_ERROR_POPUP_START_HEIGHT)),
                                      ImVec2(INFINITY, GET_UISIZE(PROJECT_SAVE_ERROR_POPUP_START_HEIGHT)));
